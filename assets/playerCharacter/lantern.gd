...
@onready var oil_lantern = get_node(
	"Sketchfab_Scene/Sketchfab_model/4b17dd9eba0245778cead7ee8fe19576_fbx/RootNode/Oil Lantern"
)
...
func _input(_event: InputEvent) -> void:
	if Input.is_action_just_pressed("lantern"):
		...
		tween.tween_method(
			oil_lantern.set_lit, 0.0 if lantern_on else 1.0, 1.0 if lantern_on else 0.0, 0.2
		)
